  ...
  return (
    <PageShell currentPath={currentPath}>
      <section className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-card)] md:p-8">
        ...
        <h1 className="mt-2 text-3xl font-extrabold text-[var(--color-text)]">{title}</h1>
        ...
        {isFormacoesRoute && selectedTrilha && !selectedModulo && !isCompleted && (
          <div className="mt-8 space-y-6">
            <button
              ...
              className="rounded-[6px] border border-[var(--color-border)] px-4 py-2 text-sm font-medium text-[var(--color-text)] transition hover:bg-[color:rgba(79,70,229,0.06)]"
            >
              ← Voltar para trilhas
            </button>
            ...
            <div className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div className="rounded-[8px] border border-[var(--color-border)] bg-[color:rgba(79,70,229,0.06)] p-4">
                  ...
                  <p className="mt-1 text-3xl font-extrabold text-[var(--color-primary)]">{progressValue}%</p>
                </div>
              </div>
                ...
                {selectedTrilha.modulos.map((modulo) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex flex-col gap-3 rounded-[8px] border border-[var(--color-border)] p-4 transition hover:border-[var(--color-primary)] md:flex-row md:items-center md:justify-between"
                    >
                      <div>
        ...
        {isFormacoesRoute && selectedTrilha && isCompleted && (
          <div className="mt-8 space-y-6">
            <div className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-8 text-center shadow-sm">
              <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[color:rgba(79,70,229,0.10)] text-[var(--color-primary)]">
                <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 2 4 5v6c0 5 3.4 8.8 8 11 4.6-2.2 8-6 8-11V5l-8-3Z" fill="currentColor" />
                </svg>
              </div>
              <h2 className="mt-6 text-3xl font-bold text-[var(--color-text)]">Parabéns!</h2>
              <p className="mt-3 text-[var(--color-muted)]">
                Você concluiu todos os módulos desta trilha obrigatória.
              </p>
              <div className="mt-6 inline-flex rounded-full border border-[var(--color-primary)] bg-[color:rgba(79,70,229,0.08)] px-5 py-3 text-sm font-semibold text-[var(--color-primary)]">
                {selectedTrilha.badge_recompensa}
              </div>
              <p className="mt-6 text-sm font-medium text-[var(--color-text)]">Status da trilha: Concluída</p>
            </div>
          </div>
        )}
            ...
            <div className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
              <div className="space-y-6">
                ...
                {selectedModulo.tipo === 'video' ? (
                  ...
                  <div className="rounded-[8px] border border-[var(--color-border)] bg-[color:rgba(79,70,229,0.04)] p-5 text-sm leading-7 text-[var(--color-muted)]">
                    <p>
                    ...
                    <p className="mt-3">
                     ...
                     <ReactMarkdown>{selectedModulo.conteudo_texto}</ReactMarkdown>
                  </div>
                )}

                <div className="space-y-4 rounded-[8px] border border-[var(--color-border)] bg-[color:rgba(79,70,229,0.04)] p-5">
                  <h3 className="text-lg font-semibold text-[var(--color-text)]">Questionário</h3>
                  {selectedModulo.questionario.map((question, index) => (
                    <div key={`${selectedModulo.modulo_id}-${index}`} className="space-y-2 rounded-[8px] bg-[var(--color-surface)] p-4">
                      <p className="text-sm font-semibold text-[var(--color-text)]">{question.pergunta}</p>
                      <div className="space-y-2">
                        {question.opcoes.map((option, optionIndex) => (
                          <label key={`${selectedModulo.modulo_id}-${index}-${optionIndex}`} className="flex items-center gap-2 rounded-[6px] border border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-muted)]">
                            <input
                              type="radio"
                  ...
                  {quizCompleted && (
                    <div className="rounded-[8px] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 text-sm text-[var(--color-text)]">
