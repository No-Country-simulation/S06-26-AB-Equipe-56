...
// Registra as ferramentas (tools) e prompts do MCP em um server já criado.
async function registerMcpTools(server) {
  ...
  const formatarErro = (erro, acao) => {
    console.error(`Erro ao ${acao}:`, erro); // Loga no stderr para debug
    return {
...
// Cria o McpServer, registra as tools e conecta via STDIO.
